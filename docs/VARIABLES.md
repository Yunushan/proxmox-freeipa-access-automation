...
- `linux_freeipa_enroll_apt_lock_timeout`: seconds the Debian-family APT metadata refresh waits for package-manager locks before failing; useful when cloud-init or unattended upgrades are still finishing on a fresh guest; defaults to `300`
