...
- `linux_freeipa_enroll_split_dns_public_dns_servers`: optional explicit public/default DNS server list used by the split-DNS automation. When empty, Linux enrollment auto-detects the current active-link DNS servers when that link is not already carrying the route-only IPA domain and falls back to the current global DNS servers otherwise; defaults to `[]`
