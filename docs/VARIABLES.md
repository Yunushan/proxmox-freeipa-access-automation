...
- `linux_freeipa_enroll_cloud_init_timeout`: seconds Debian-family Linux enrollment waits for cloud-init completion before continuing to the generic APT lock wait path; defaults to `900`
- `linux_freeipa_enroll_wait_for_apt_background_services`: when `true`, Debian-family Linux enrollment waits for `apt-daily.service` and `apt-daily-upgrade.service` to become inactive on systemd guests before refreshing APT metadata; the long-lived `unattended-upgrades` shutdown helper is not treated as a blocker by itself because the following APT lock wait already catches real package-manager contention; defaults to `true`
- `linux_freeipa_enroll_apt_background_services_timeout`: seconds Debian-family Linux enrollment waits for those background package services before continuing to the generic APT lock wait path; defaults to `900`
- `linux_freeipa_enroll_apt_lock_timeout`: seconds Debian-family Linux enrollment waits for active `apt` or `dpkg` lock holders before failing the package-metadata refresh; useful when cloud-init or unattended upgrades are still finishing on a fresh guest; defaults to `300`
