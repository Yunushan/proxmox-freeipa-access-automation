...
- `guest_qemu_agent_install_reboot_after_proxmox_vm_agent_enable`: when `true`, Linux workflows reboot running Proxmox-backed VMs after enabling the Proxmox guest-agent option; leave it `false` if you do not want the repository to reboot those guests automatically; defaults to `false`
