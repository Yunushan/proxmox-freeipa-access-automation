...
- `linux_freeipa_enroll_split_dns_public_dns_servers`: optional explicit public/default DNS server list used by the split-DNS automation. When empty, Linux enrollment auto-detects the current active-link DNS servers after clearing any old per-link `systemd-resolved` overrides and falls back to the current global DNS servers only when the link has no usable public resolvers; defaults to `[]`
