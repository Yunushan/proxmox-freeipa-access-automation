...
- `linux_freeipa_enroll_wait_for_apt_background_services`: when `true`, Debian-family Linux enrollment waits for `apt-daily.service`, `apt-daily-upgrade.service`, and `unattended-upgrades.service` to become inactive on systemd guests before refreshing APT metadata; defaults to `true`
