...
- `linux_freeipa_enroll_split_dns_public_dns_servers`: optional explicit public/default DNS server list used by the split-DNS automation. In the default `explicit` mode, set this to the public resolver IPs you want the guest to keep globally. If you leave it empty, the role now reuses the guest's currently detected non-IPA public/default resolvers when available. In `preserve` mode, leaving it empty keeps the fully automatic preserve-and-recover behavior; defaults to `[]`
