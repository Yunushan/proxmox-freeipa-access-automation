...
- `linux_freeipa_enroll_check_local_user_conflicts`: when `true`, Linux enrollment also compares the local `/etc/passwd` usernames on the guest against the effective IPA members of the configured admin groups and surfaces exact-name collisions before the repository writes local SSH and sudo access policy; defaults to `true`
- `linux_freeipa_enroll_fail_on_local_user_conflicts`: when `true`, any exact-name collision between a local `/etc/passwd` user and an IPA-backed Linux admin user fails the enrollment run instead of emitting a warning; defaults to `false`
- `linux_freeipa_enroll_local_user_conflict_groups`: IPA groups used for that local-user collision check; defaults to `linux_freeipa_enroll_local_sudo_groups`
