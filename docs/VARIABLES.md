...
- `linux_freeipa_enroll_local_sudo_group_domain_variants`: when `true`, Linux enrollment also writes IPA-qualified variants of those groups such as `linux-ssh-admins@karel.local` and `linux-ssh-admins@KAREL.LOCAL` to the local sudoers drop-in for guests that expose SSSD groups in qualified form; defaults to `true`
