...
- `linux_freeipa_enroll_recover_stuck_apt_daily`: when `true`, Debian-family Linux enrollment terminates a stale `apt-daily.service` update job after the APT lock wait times out, then retries the lock check instead of failing immediately; defaults to `true`
