...
- `linux_freeipa_enroll_split_dns_public_dns_servers` defaults to an empty list, so the role auto-detects the current active-link DNS servers after clearing any old per-link `systemd-resolved` overrides and falls back to the current global DNS servers only when the link has no usable public resolvers. Set it explicitly if you want fixed public resolver IPs instead of automatic detection.
