...
- `linux_freeipa_enroll_recover_stuck_apt_daily` defaults to `true`, so when the Debian APT lock wait times out and `apt-daily.service` is still active, the role terminates that stale background update job and retries the lock check instead of failing immediately.
