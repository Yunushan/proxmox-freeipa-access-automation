...
- `linux_freeipa_enroll_split_dns_public_dns_servers` defaults to an empty list. In the default `explicit` mode, an empty list now falls back to the guest's recoverable non-IPA public/default resolvers, starting with the current runtime view and then trying reverted link state, netplan, and guest facts before failing. An explicit list is still the safer and more deterministic choice.
