...
- `linux_freeipa_enroll_apt_lock_timeout` defaults to `300`, so the APT metadata refresh will wait longer for transient package-manager locks from cloud-init, unattended upgrades, or other guest bootstrap activity before failing the enrollment.
