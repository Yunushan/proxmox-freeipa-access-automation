...
- `linux_freeipa_enroll_wait_for_apt_background_services` defaults to `true`, so Debian-family systemd guests also wait for `apt-daily` and `apt-daily-upgrade` to stop before touching APT metadata. The long-lived `unattended-upgrades` shutdown helper is not treated as a blocker on its own; actual APT lock contention is still enforced by the following lock-holder wait.
