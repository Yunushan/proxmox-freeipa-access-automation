...
- Linux QGA SSH bootstrap now also skips guests whose agent responds to `guest-ping` but cannot launch the configured in-guest exec shell, and exposes `linux_ipa_qga_ssh_bootstrap_exec_shell` plus `linux_ipa_qga_ssh_bootstrap_fail_on_exec_shell_missing` for stricter or custom-shell environments.
