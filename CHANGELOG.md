...
- FreeIPA login-shell reconciliation now parses both direct and indirect group members, and also normalizes comma-separated `ipa group-show --raw` member output before enforcing `/bin/bash` on Linux admin users.
